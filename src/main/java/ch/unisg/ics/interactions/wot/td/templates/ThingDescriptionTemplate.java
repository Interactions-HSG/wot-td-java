...
public class ThingDescriptionTemplate implements Template{
  ...
  public boolean isTemplateOf(Object obj) {
    boolean b = true;
    ThingDescription td = (ThingDescription) obj;
    for (ActionAffordance action: td.getActions()){
      Optional<ActionAffordanceTemplate> actionTemplate =  getActionByName(action.getName());
      if (!actionTemplate.isPresent()){
        b = false;
      } else if (!actionTemplate.get().isTemplateOf(action)){
        b = false;
      }
    }
    return b;
